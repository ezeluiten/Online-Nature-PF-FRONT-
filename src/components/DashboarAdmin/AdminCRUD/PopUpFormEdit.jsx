...
console.log(handleUpdate, "hola")
        ...
        <div className={style.formPopup}>
          ...
          <div>
            ...
            <h2 className={style.titleForm}>Update form</h2>
                    ...
                    <input className={`${error.title} && ${style.warning} ${style.inputForm}`} value={formData.title} onChange={handleInputChange} type="text" id="title" name="title" placeholder="ej: Pig"/>
                    <p className={style.danger}>{error.title}</p>
                    <br />
                    
                    <label htmlFor="image">Image</label>
                    <input className={`${error.image} && ${style.warning} ${style.inputForm}`} value={formData.image} onChange={handleInputChange} type="text" id="image" name="image" placeholder="URL"/>
                    <p className={style.danger}>{error.image}</p>
                    <br />

                    <label htmlFor="image_detail">Image detail:</label>
                    <input className={`${error.image_detail} && ${style.warning} ${style.inputForm}`} value={formData.image_detail} onChange={handleInputChange} type="text" id="image_detail" name="image_detail" placeholder="url"/>
                    <p className={style.danger}>{error.image_detail}</p>
                    <br />
                    
                    <label htmlFor="description">Description:</label>
                    <textarea className={`${error.description} && ${style.warning} ${style.inputForm}`} value={formData.description} onChange={handleInputChange} type="text"  id="description" name="description" placeholder="Description"/>
                    <p className={style.danger}>{error.description}</p>
                    <br />

                    <label htmlFor="amount">Amount:</label>
                    <input className={`${error.amount} && ${style.warning} ${style.inputForm}`} value={formData.amount} onChange={handleInputChange} type="number" id="amount" name="amount" placeholder="ej: 30"/>
                    <p className={style.danger}>{error.amount}</p>
                    <br />
              <div className={style.buttonsForm}>
                <Button type="submit" variant="success">Update</Button>{' '}
                <Button variant="warning" onClick={handleUpdate}>Go back</Button>{' '}
              </div>
            </form>
          </div>
        </div>
  ...
  //     {showFormEdit && (<div className={style.formPopup}>
